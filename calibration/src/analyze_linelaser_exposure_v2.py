#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""评估不同短曝光下棋盘格表面激光线的提取质量。\n\n长曝光棋盘格图仅用于几何定位；只有同曝光 laser-off 图才参与差分。\n"""
...
def parser() -> argparse.ArgumentParser:
    ...
    p.add_argument('--background-root', type=Path,
                   help='可选：同姿态同曝光的短曝光激光关闭图像根目录，仅用于 on-off 差分')
    p.add_argument('--board-root', type=Path,
                   help='可选：长曝光、激光关闭的棋盘格图像根目录，仅用于角点和黑白格几何定位')
    p.add_argument('--board-group', default='20000us',
                   help='--board-root 下使用的曝光文件夹，默认 20000us')
...
def board_reference_path(root: Path, board_group: str, rel: Path):
    """将 300us/pose_01.tif 映射为 board_root/20000us/pose_01.tif。"""
    folder = root / board_group
    exact = folder / rel.name
    if exact.is_file():
        return exact
    if not folder.is_dir():
        return None
    matches = [
        ...
        and p.stem.lower() == rel.stem.lower()
    ]
...
def main():
    ...
    board_root = args.board_root.resolve() if args.board_root else None
    if not root.is_dir():
        print(f'[错误] 输入目录不存在：{root}', file=sys.stderr); return 1
    if bg_root and not bg_root.is_dir():
        print(f'[错误] 短曝光背景目录不存在：{bg_root}', file=sys.stderr); return 1
    if board_root and not board_root.is_dir():
        print(f'[错误] 长曝光棋盘格目录不存在：{board_root}', file=sys.stderr); return 1
    files = images(root)
    if not files:
    ...
    for idx, path in enumerate(files, 1):
        ...
        board_ref = None
        board_path = (
            board_reference_path(board_root, args.board_group, rel)
            if board_root is not None else None
        )
        if board_path is not None:
            board_img = read_img(board_path)
            if board_img is not None:
                board_ref = gray(board_img)
                if board_ref.shape != on.shape:
                    print(f'[警告] 长曝光棋盘格图尺寸不一致，忽略：{board_path}')
                    board_ref = None

        # 长曝光图只用于几何定位；同曝光 laser-off 图才可用于灰度差分。
        reference = board_ref if board_ref is not None else (off if off is not None else on)
        reference_maximum = max_dn(reference, args.max_value)
        found, corners = chessboard(
            u8(reference, reference_maximum), (args.cols, args.rows)
        )
        base = dict(
            group=group,
            exposure_us=exposure_us(group),
            filename=str(rel),
            background_found=int(off is not None),
            board_reference_found=int(board_ref is not None),
            board_detected=int(found),
        )
        if not found:
        ...
        classes = surface_classes(reference, mask, reference_maximum)
    ...
    fields = ['group','exposure_us','filename','background_found',
              'board_reference_found','board_detected',
    ...
    for group, rs in grouped.items():
        ...
        s = dict(group=group, exposure_us=exposure_us(group), image_count=len(rs),
                 ...
                 board_reference_rate=finite([r['board_reference_found'] for r in rs], 'mean'),
