#!/usr/bin/env python3
"""支持交互旋转查看的 Mono8 单图/目录线激光三维截面重建入口。"""
...
DEFAULT_CONFIG = (
    Path(__file__).resolve().parents[1] / "config" / "reconstruct_ground_pointcloud_v3.yaml"
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--show-3d",
        action="store_true",
        help="保存结果后弹出可旋转的 Matplotlib 3D 窗口",
    )
    return parser.parse_args(argv)


def configure_plotting(show_3d: bool) -> None:
    """在首次导入 pyplot 前选择后台或交互后端。"""
    global plt
    if plt is not None:
        return
    try:
        matplotlib.use("TkAgg" if show_3d else "Agg", force=True)
        import matplotlib.pyplot as pyplot
    except (ImportError, RuntimeError) as exc:
        mode = "交互式 TkAgg" if show_3d else "Agg"
        raise ReconstructionError(f"无法启用 Matplotlib {mode} 后端：{exc}") from exc
    plt = pyplot
...
def load_calibration(
    ...
    coordinate_unit = plane_yaml.get("coordinate_unit", plane_yaml.get("units", ""))
    if str(coordinate_unit).lower() != "mm":
        raise ReconstructionError("laser_plane.yaml 的 coordinate_unit/units 必须为 mm")
    coeffs = plane_yaml.get(
        "coefficients",
        plane_yaml.get("plane", plane_yaml.get("laser_plane", plane_yaml)),
    )
    if isinstance(coeffs, dict) and "coefficients" in coeffs:
        coeffs = coeffs["coefficients"]
    try:
        if isinstance(coeffs, dict):
            lower = {str(key).lower(): value for key, value in coeffs.items()}
            laser_plane = np.asarray(
                [lower[key] for key in ("a", "b", "c", "d")], dtype=np.float64
            )
        else:
            laser_plane = np.asarray(coeffs, dtype=np.float64).reshape(-1)
    except (KeyError, TypeError, ValueError) as exc:
        raise ReconstructionError("激光平面必须包含数值 a、b、c、d") from exc
    if laser_plane.size != 4:
        raise ReconstructionError("激光平面必须包含 4 个系数 [a,b,c,d]")
...
def show_interactive_pointcloud(frame: pd.DataFrame, title: str) -> None:
    """显示阻塞式 3D 窗口；关闭窗口后程序继续。"""
    ...
    axis.zaxis.set_major_locator(MaxNLocator(7))
    axis.set_xlabel("Xg (mm)")
    axis.set_ylabel("Yg (mm)")
    axis.set_zlabel("Zg (mm)")
    axis.set_title(f"Interactive 3D | {title}")
    axis.view_init(elev=24.0, azim=-58.0)
    axis.text2D(
        0.02,
        0.98,
        "Drag to rotate | Mouse wheel to zoom | Close window to continue",
        transform=axis.transAxes,
        va="top",
    )
    manager = getattr(figure.canvas, "manager", None)
    if manager is not None and hasattr(manager, "set_window_title"):
        manager.set_window_title(f"Line laser point cloud - {title}")
    figure.tight_layout()
    plt.show(block=True)
    plt.close(figure)
...
def process_frame(
    ...
    if show_3d:
        show_interactive_pointcloud(frame, image_path.name)
    return statistics
...
def run(args: argparse.Namespace) -> int:
    ...
    configure_plotting(args.show_3d)
    ...
    if args.show_3d:
        print("交互 3D 已启用：拖动鼠标旋转，滚轮缩放，关闭窗口后继续。")
