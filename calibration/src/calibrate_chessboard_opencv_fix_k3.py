...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=[],
        metavar="FILENAME",
        help="Exact image filenames to exclude before detection",
    )
    parser.add_argument(
        "--fix-k3",
        action="store_true",
        help="Fix the fifth distortion coefficient k3 to exactly zero",
    )
...
def calibrate(args: argparse.Namespace) -> None:
    ...
    available_by_name = {path.name.casefold(): path for path in image_paths}
    excluded_names = {name.casefold() for name in args.exclude_images}
    unknown_exclusions = excluded_names - available_by_name.keys()
    if unknown_exclusions:
        unknown = ", ".join(sorted(unknown_exclusions))
        raise ValueError(f"排除列表中的文件不在候选图像中：{unknown}")
    image_paths = [
        path for path in image_paths if path.name.casefold() not in excluded_names
    ]
    if not image_paths:
        raise ValueError("排除指定图像后没有可用于标定的图像")
    for name in args.exclude_images:
        print(f"[排除] {name}")
    ...
    calibration_flags = cv2.CALIB_FIX_K3 if args.fix_k3 else 0
    ...
    if args.fix_k3 and float(dist_coeffs.reshape(-1)[4]) != 0.0:
        raise RuntimeError("CALIB_FIX_K3 已启用，但最终 k3 不为 0")
