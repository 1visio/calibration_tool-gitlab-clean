...
def _read_opencv_filestorage(path: Path) -> tuple[Optional[np.ndarray], Optional[np.ndarray]]:
    ...
    try:
        K = None
        D = None
        for key in ("camera_matrix", "K", "cameraMatrix"):
            node = fs.getNode(key)
            if not node.empty():
                matrix = node.mat()
                if matrix is not None and matrix.size == 9:
                    ...
                    break
        for key in ("dist_coeffs", "distortion_coefficients", "D", "distCoeffs"):
            node = fs.getNode(key)
            if not node.empty():
                matrix = node.mat()
                if matrix is not None and matrix.size >= 4:
                    D = np.asarray(matrix, dtype=np.float64).reshape(-1)
                    break
        return K, D
    finally:
        fs.release()
...
def load_reference_intrinsics(path: Path) -> tuple[np.ndarray, np.ndarray]:
    ...
    if K is None or D is None:
        raise ValueError(
            "内参 YAML 中未找到 camera_matrix/K 和 dist_coeffs/D；"
            "支持列表格式或含 data 字段的 OpenCV 格式。"
        )
...
def main() -> int:
    ...
    if args.reference_yaml is not None:
        ...
        if not reference_path.exists():
            print(f"错误：内参文件不存在：{reference_path}", file=sys.stderr)
            return 2
        try:
        ...
        try:
            ...
            print(f"错误：内部快速标定失败：{exc}", file=sys.stderr)
            return 2
