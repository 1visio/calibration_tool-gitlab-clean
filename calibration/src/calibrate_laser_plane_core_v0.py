#!/usr/bin/env python3
"""Core, leakage-safe line-laser plane calibration from chess/laser pairs.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--train-ids", type=common.parse_id_spec, default="1-25")
    parser.add_argument("--val-ids", type=common.parse_id_spec, default="26-32")
...
def validate_arguments(args: argparse.Namespace) -> None:
    ...
    if not train or not validation:
        raise common.CalibrationError("训练集和验证集都不能为空")
    if train & validation:
        raise common.CalibrationError("训练集和验证集编号不得重叠")
...
def validate_inputs(
    ...
    for split, identifiers in (("train", args.train_ids), ("validation", args.val_ids)):
        for image_id in identifiers:
            chess = image_path(args.image_dir, args.chess_pattern, image_id)
            laser = image_path(args.image_dir, args.laser_pattern, image_id)
            for path in (chess, laser):
                if not path.is_file():
                    raise FileNotFoundError(f"缺少配对图像：{path}")
                image = cv2.imread(str(path), cv2.IMREAD_UNCHANGED)
                if image is None:
                    raise common.CalibrationError(f"无法读取图像：{path}")
                shape = image.shape[:2]
                if shape != expected:
            ...
            pairs.append((image_id, split, chess, laser))
    LOGGER.info(
        "输入检查通过：%d 个训练对，%d 个验证对，尺寸=%s",
        len(args.train_ids),
        len(args.val_ids),
...
def prepare_frame(
    ...
    chess = common.to_gray(common.read_image(chess_path), chess_path)
    laser = common.to_gray(common.read_image(laser_path), laser_path)
    ...
    window = int(nested(config, "laser.background_window_px"))
    percentile = float(nested(config, "laser.background_percentile")) * 100.0
    ...
    smoothed = gaussian_filter1d(corrected, sigma=sigma, axis=0, mode="nearest")
    raw_smoothed = gaussian_filter1d(image, sigma=sigma, axis=0, mode="nearest")
...
def measure_pair_motion(
    ...
    if not bool(nested(config, "pair_motion.enabled")):
        return {"tracking_ok": False, "movement_method": "disabled"}
    ...
    patch = int(nested(config, "pair_motion.template_patch_radius_px"))
    search = int(nested(config, "pair_motion.template_search_radius_px"))
    minimum_correlation = float(nested(config, "pair_motion.template_min_correlation"))
    ...
    for corner_x, corner_y in prepared.pose.corners:
        ...
        if (
            ...
            continue
        template = chess[y - patch : y + patch + 1, x - patch : x + patch + 1]
        region = laser[y - margin : y + margin + 1, x - margin : x + margin + 1]
    ...
    if not shifts:
        return {
            "tracking_ok": False,
            "movement_method": "unresolved",
            "template_consensus_ratio": 0.0,
    ...
    consensus = np.linalg.norm(values - centre, axis=1) <= float(
        nested(config, "pair_motion.template_consensus_radius_px")
    )
    ratio = float(np.count_nonzero(consensus) / len(prepared.pose.corners))
    ok = ratio >= float(nested(config, "pair_motion.template_min_consensus_ratio"))
    if not ok:
        return {
            "tracking_ok": False,
            "movement_method": "unresolved",
    ...
    return {
        "tracking_ok": True,
        "movement_method": "local_template",
        "template_consensus_ratio": ratio,
...
def process_pair(
    ...
    motion = measure_pair_motion(prepared, config)
    LOGGER.info(
    ...
    return CoreFrame(prepared, result, motion)
...
def auto_excluded_ids(
    ...
    if not bool(nested(config, "pair_motion.auto_exclude_training")):
        return excluded
    threshold = float(nested(config, "pair_motion.severe_px"))
    for frame in train_frames:
        movement = frame.pair_motion.get("median_displacement_px", math.nan)
        if frame.pair_motion.get("tracking_ok") and float(movement) > threshold:
...
def build_summaries(
    ...
    frame_by_id = {frame.result.image_id: frame for frame in frames}
    pose_rows: list[dict[str, Any]] = []
    for (split, image_id), group in points.groupby(["split", "image_id"], sort=True):
        core = frame_by_id[int(image_id)]
        ...
        if not core.pair_motion.get("tracking_ok", False):
...
def draw_centres(image: np.ndarray, xy: np.ndarray) -> np.ndarray:
    ...
    for x, y in ordered[::marker_step]:
        cv2.circle(canvas, (int(round(x)), int(round(y))), 3, (255, 0, 255), -1, cv2.LINE_AA)
    return canvas
...
- `feature_points_plane_train.png` 与 `feature_points_plane_validation.png` 左侧为三维特征点和平面，右侧为逐姿态点面距离曲线。
"""
...
def run(args: argparse.Namespace) -> np.ndarray:
    ...
    LOGGER.info("阶段 4/4：保存距离、姿态指标、叠加图和特征点平面图")
    write_outputs(
        args,
        config,
        intrinsics,
