...
5. 标定完成或生成验收报告后进入第 5 页，在左侧结果树选择 stage、fit/validation、pose；中间查看已有图像，右侧查看路径、可可靠关联的指标及 CSV 曲线。
6. “上一张/下一张”按当前筛选浏览结果；“在文件夹中打开”打开对应目录。图像损坏或缺失只会显示提示，不会使结果页崩溃。没有逐图图像的阶段只显示已有汇总图和诊断文件。
...
7. 在“逐图结果浏览”中按 stage、split、pose_id 和状态筛选；选择 `fit` 或 `validation` 下的 pose 查看已有 extraction 图。
8. `validation_error_vs_u.png`、`validation_error_vs_v.png`、`validation_error_vs_depth.png` 等汇总图位于 `summary_plots` 节点；CSV/YAML/JSON 诊断文件会在右侧显示文件信息和可匹配的指标。
9. 点击“上一张/下一张”连续检查；指标无法可靠匹配时显示“暂无关联指标”，不会猜测或改写原始结果。

内参阶段如果算法没有输出逐图角点或重投影 PNG，结果页不会伪造图像，只列出该阶段已有的 YAML、CSV 或汇总诊断产物。
